...
public class OpenbisV3CreationController implements IOpenbisCreationController {
  ...
  private OpenbisV3ReadController openbis;
  ...
  public OpenbisV3CreationController(OpenbisV3ReadController openbis, String user,
  ...
  public boolean registerExperiment(String space, String project, ExperimentType expType,
    ...
    errors = "";
    if (!openbis.projectExists(project)) {
  ...
  public boolean registerExperiments(String space, String proj, List<RegisterableExperiment> exps) {
    ...
    for (RegisterableExperiment e : exps) {
      if (!openbis.experimentWithIDExists(
          ExperimentCodeFunctions.getExperimentIdentifier(space, proj, e.getCode()))) {
        ExperimentCreation exp = new ExperimentCreation();
  ...
  public void registerProjectWithExperimentsAndSamplesBatchWise(
    ...
    for (String experiment : entitiesToUpdate.keySet()) {
      ...
      Date modificationDate = openbis.getExperimentModificationDate(expID);
      ...
      while (newModificationTime == modificationTime && TIMEOUT > 0) {
        Date newModificationDate = openbis.getExperimentModificationDate(expID);
  ...
  public void updateDatasets(Map<String, Map<String, String>> idsToProps) {
    ...
    for (String permID : idsToProps.keySet()) {
      ...
      Map<String, String> map = idsToProps.get(permID);
      dsUpdate.setProperties(map);
